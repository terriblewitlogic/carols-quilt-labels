...
- Added unsafe legacy structural safe-flip filtering:
  - legacy safe flips are suppressed when they introduce an extreme internal underlay-to-cover handoff and the reoriented flip removes internal risk
  - `same_hue_shell_facets` same-surface trimmed long spans improved `1 -> 0`
  - jumps/trims stayed stable, quality stayed `100`, and broad uploaded/generated/underpaint comparisons had no top-line metric deltas
...
- Added sparse outline-halo source accounting:
  - `thick_outline_flower` no longer treats the pruned gray outline antialias halo as semantic color loss
  - source suitability improved `76 / candidate -> 100 / clean`, repair opportunities `1 -> 0`
  - stitch output stayed unchanged at `6952` stitches, `54` jumps, `6` trims
- Added stitched near-white foreground accounting:
  - `bee_simple`, `flower_daisy_simple`, and `low_contrast_bird` now count preserved pale foreground colors as accounted source evidence instead of repair opportunities
  - source repair opportunities improved `1 -> 0` for those three fixtures
  - stitch output, route diagnostics, and quality scores stayed unchanged
- Reduced broad dark stroke-satin overpacking:
  - large dark outline networks at or above `250mm²` use a wider density multiplier while small/narrow linework keeps the denser multiplier
  - `real_teapot_card` stitches improved `13541 -> 11761` and trims `15 -> 14`
  - `thick_outline_flower` stitches improved `6952 -> 6122`
  - `cartoon_elephant` stitches improved `5725 -> 5191`
  - full uploaded/generated/source-color/underpaint comparisons passed without guarded quality, acceptance, stitched-span, high-risk, or broad-route regressions
- Added local patch serpentine fill:
  - simple local material patches from `90-220mm^2` can switch to a proved-safe serpentine fill when the candidate has no long gaps or trim gaps
  - `real_teapot_card` stitches improved `11761 -> 11711`
  - `thick_outline_flower` stitches improved `6122 -> 6113`
  - `sparrow_flat_app_icon` stitches improved `5488 -> 5483`
  - `synthetic_structural_route_facets` stitches improved `5883 -> 5792`
...
- Added cross-color compact tiny detail accounting:
  - source repeated-motif detection now recognizes stitched compact dot motifs spread across multiple colors
  - `tiny_detail_icon` source repair opportunities improved `1 -> 0`
...
- Added repeated compact motif repair accounting:
  - source diagnostics suppress generic `fragmented_line_art` repair opportunities when the same stitched label is already classified as a same-color compact detail repeat
  - `real_strawberry` source repair opportunities improved `1 -> 0`
  - stitch output stayed unchanged at `7369` stitches, `65` jumps, `3` trims, quality `100`, with preserved green/red/orange/black colors
  - repeated motif accounting stayed `2` groups / `26` components, while source suitability intentionally stayed `candidate 88` with `many_regions`
  - full source-color/uploaded/generated/underpaint comparisons passed without guarded quality, acceptance, stitched-span, high-risk, or broad-route regressions
  - validation included `PYTHONPYCACHEPREFIX=tmp/pycache npm run check:python`, `npm run typecheck`, full uploaded strict source policy, full generated acceptance `--strict`, full source-color acceptance, full underpaint benchmark, and regression-gated compare reports:
...
- Improved satin-width local material panels:
  - local material panels in teapot-like multi-color detail clusters can now attempt the existing proved-safe local serpentine path even when geometry is satin-width
  - `surface-plan.json` records local-patch rejection reasons for material panels, while stem-like, holed, silhouette, center-disk, and untagged satin-zone surfaces stay blocked
  - `real_teapot_card` fill-coherence risk improved `1 -> 0`
  - local-patch-serpentine material panels improved `5 -> 8`, scan/solid-scan material panels reduced `4 -> 1`, and stitches improved `11664 -> 11612`
  - jumps stayed `86`, trims stayed `14`, quality stayed `100`, and same-surface stitched/untrimmed long-span, high-risk, and broad-route-risk gates stayed `0`
  - full source-color/uploaded/generated/underpaint comparisons passed without guarded quality, acceptance, stitched-span, high-risk, or broad-route regressions
  - validation included `PYTHONPYCACHEPREFIX=tmp/pycache npm run check:python`, `npm run typecheck`, full uploaded strict source policy, full generated acceptance `--strict`, full source-color acceptance, full underpaint benchmark, and regression-gated compare reports:
...
## Satin-Width Local Material Panels

The teapot source-color fixture now clears its remaining fill-coherence risk without changing public interfaces or broad route behavior. Local material panels inside multi-color detail clusters may try the already scored local serpentine path even when the geometry is satin-width; non-material satin zones and stem-like panels stay blocked.

Accepted 2026-06-28 result:

- `real_teapot_card` fill-coherence risk `1 -> 0`
- local material-panel serpentine count `5 -> 8`
- scan/solid-scan material panels `4 -> 1`
- stitches `11664 -> 11612`
- jumps `86 -> 86`, trims `14 -> 14`, quality `100`
- same-surface stitched/untrimmed long-span, high-risk, and broad-route-risk gates stayed `0`

Validation passed with full source-color, uploaded strict source policy, generated strict acceptance, underpaint benchmark, Python checks, TypeScript typecheck, and regression-gated comparisons:
...
Next source/color work should move beyond teapot unless there is a new visible output failure; the better target is a real uploaded/generated semantic color-loss, same-hue material-collapse, or over-fragmented detail fixture.
