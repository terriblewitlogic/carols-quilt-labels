...
**Backend quality checkpoint — 2026-06-30:** the stitch backend now has source-color acceptance lanes for real generated art before this is exposed to users. `real_strawberry` is accounted as intentional compact repeated detail, `real_teapot_card` is kept as a source-complexity guard with `4` retained multi-component colors / `22` disconnected retained pieces, `thick_outline_flower` now treats pruned gray outline antialiasing as sparse halo residue instead of semantic color loss, and `tiny_detail_icon` now treats multi-color compact dots as intentional source motifs (`candidate 88 -> clean 100`, repair opportunities `1 -> 0`) while preserving stitch output. Recent visible output wins reduced broad dark outline density, cleaned simple local material fills, reduced repeated seed-field overpacking, tightened teapot-like local material panels, added compact pastel accent preservation, added bookended same-hue material preservation, added muted chromatic thread-snap preservation, and added saturated dark green/red same-hue material preservation without guarded risk regressions: `real_teapot_card` stitches `13541 -> 11761 -> 11711 -> 11664 -> 11612`, `thick_outline_flower` `6952 -> 6122 -> 6113`, `cartoon_elephant` `5725 -> 5191`, `sparrow_flat_app_icon` `5488 -> 5483`, and `real_strawberry` `7549 -> 7369` with trims `4 -> 3` and preserved colors. New uploaded guard `muted_flower_pin` preserves lavender `#b496dc`, yellow `#ffaf46`, blue fill, and black details with quality `100`, detail budget `ok`, and explicit compact-pastel accounting. New uploaded guard `same_hue_purple_shell_facets` preserves dark/mid/light purple `#7846c8`, `#965ad2`, `#b496dc`, and black; the no-guard baseline dropped dark purple, while the accepted run improves trims `6 -> 4`, jumps `20 -> 19`, and same-surface trimmed spans `1 -> 0` with quality `100`. New uploaded guard `muted_sage_detail_badge` keeps a small sage detail as chromatic Madeira `#1ea096` instead of neutral `#808080`, with quality `100`, `4147` stitches, `26` jumps, `2` trims, and no existing-case movement in uploaded/generated/source-color/underpaint comparisons. New uploaded stress guard `same_hue_green_leaf_facets` keeps saturated dark/mid/light green `#144614`, `#3ca03c`, `#96dc82`, and black; the probe baseline dropped dark green, while the accepted run improves stitches `5370 -> 4873`, jumps `10 -> 8`, trims `2 -> 1`, and same-surface trimmed spans `1 -> 0` with quality `100`. New uploaded stress guard `same_hue_red_shell_facets` keeps saturated dark/mid/light red `#8c000a`, `#c83264`, `#f082a0`, and black; the probe baseline collapsed dark red into mid red, while the accepted run keeps quality `100`, `4444` stitches, `15` jumps, `6` trims, no same-surface relocations, and no broad-route risk. New source/color guards also cover a single tiny vivid accent (`tiny_vivid_accent_badge`: quality `100`, `3188` stitches, `13` jumps, `0` trims, source tiny `1 -> accounted 1`) and gold same-hue facets (`same_hue_gold_shell_facets`: quality `100`, preserved `#b48200`, `#e6be14`, `#fff03c`, and black, no same-surface relocations). Same-hue thread-collision repair now preserves the `no_outline_teddy` dark ears as `#783c14` instead of collapsing them into body brown; quality/source suitability stay `100 / clean`, stitches improve `4548 -> 4009`, and the accepted tradeoff is higher jumps/trims from the extra semantic color. Generated same-hue overlay underpaint now preserves the `gradient_elephant_simple` mid-pink tone `#f082a0`; source-fidelity improves `62.1 -> 72.3`, palette agreement `0.667 -> 1`, region recall `0.808 -> 0.971`, quality stays `100`, trims stay `4`, and same-surface long spans/high-risk/broad-route risk stay `0` with no `scan_lanes`. Guarded tonal material running outlines then clean up the same elephant artifact: stitches `6050 -> 4204`, jumps `9 -> 7`, trims `4 -> 2`, source-fidelity `72.3 -> 80.4`, and near-black expansion `60.9 -> 17.9`, with no other generated/underpaint metric movement. Tiny accent detail outline suppression now fixes `sparrow_flat_app_icon`: source-fidelity `73.7 -> 83.5`, near-black expansion `62.9 -> 1.08`, stitches `5483 -> 1839`, with accepted jump/trim tradeoff `17/2 -> 33/3` and clean hard risk gates. Short-travel lane routing then reduces the remaining sparrow fill jumps: jumps `33 -> 21`, trims `3 -> 2`, stitches `1839 -> 1835`, with quality `100` and clean hard risk gates; `bee_simple` also drops stitches `1730 -> 1711` with no jump/trim/risk movement. Source visual-fidelity triage now recognizes that elephant as `review-stitch-style` rather than a remaining source/color behavior target when color, recall, source fit, and tiny/detail accounting are clean. Pattern travel attribution now records emitted jumps/trims by color/pass/type/role/route mode; that led to a gated large accent-outline internal sort that improves `flower_sunflower_simple` stitches `1819 -> 1805`, jumps `20 -> 18`, and trims `6 -> 4` while leaving daisy unchanged. Strawberry source diagnostics avoid double-counting the compact seed field as generic fragmented line-art repair pressure (`sourceRepairOpportunities 1 -> 0`) while leaving the real many-region warning intact; newer one-color seed caps remain rejected because keeping 20 seeds worsened jumps `65 -> 76` and keeping 26 worsened jumps `65 -> 67`. Teapot source triage now also accounts its handled local material panels (`9` panels, now `8` serpentine), clears fill-coherence risk `1 -> 0`, keeps jumps `86`, trims `14`, quality `100`, and preserves green/coral/purple/peach/black colors. Rejected variants remain important guardrails: the generic `70mm^2` local-patch gate churned daisy jumps, a too-aggressive `1.12` local-cluster density multiplier worsened teapot trims, carrying all preserved accent reasons through repeated compact-detail promotion regressed vivid-dot simplification, a tall faceted purple shell fixture preserved color but created broad-fill quality pressure, the first sage snap gate turned dark outline residue brown before cleanup, broad elephant same-hue preservation reintroduced `scan_lanes` and same-surface long spans, broad running outlines regressed sparrow quality `100 -> 74`, suppressing tiny accent outlines inside the same-hue elephant overlay regressed elephant jumps `7 -> 9`, broad outline internal sorting regressed daisy jumps `26 -> 29`, strawberry seed thinning worsened jumps into the `80+` range, and repeated-detail density `0.75` worsened trims `3 -> 4`. Validation included `PYTHONPYCACHEPREFIX=tmp/pycache npm run check:python`, `npm run typecheck`, targeted source/color canaries, full uploaded strict source policy, full generated acceptance, full source-color acceptance, full underpaint benchmark, and regression-gated comparisons. Current validation artifacts:
...
- Muted chromatic thread snapping now preserves small mid-tone semantic details:
  - `muted_sage_detail_badge` keeps the sage detail as chromatic Madeira `#1ea096` instead of neutral `#808080`
  - the accepted gate only applies to narrow mid-tone muted chromatic accents, so dark outline/halo residue still cleans normally
  - uploaded/generated/source-color/underpaint comparisons showed no existing-case movement
...
- find the next visible source/color behavior gap, especially same-hue over-fragmentation or semantic color loss outside the muted sage lane
- use `real_teapot_card` as a guard unless a source simplifier can visibly reduce disconnected color pieces without losing intended colors
...
- preserve meaningful accent colors without preserving noisy fragments, using strict source-policy checks; muted lavender/pink, compact low-chroma pastel, muted sage, and compact repeated-detail guard coverage are now in place
...
- `/Users/partido/jeflabelmaker/website/embroidery-stitch-backend/tmp/source_visual_fidelity_outline_block_energy_20260630/source-visual-fidelity.md`
