...
- Compact dark detail dots now use the parallel-dot satin lane up to `24mm2`, with explicit `satinDotParallel` diagnostics and uploaded strict guards. `low_contrast_bird` keeps quality `100`, jumps `24`, trims `3`, and hard risk gates clean while the eye changes from a hollow/ring-like satin detail to a filled black dot; neutral stitch movement is `2475 -> 2489`.
...
- Large final accent-outline groups now use gated internal nearest segment ordering. `flower_sunflower_simple` improves to `1805` stitches / `18` jumps / `4` trims, while `flower_daisy_simple` remains unchanged; strict generated and underpaint guards now lock sunflower at `<= 18` jumps / `<= 4` trims.

The remaining quality problems are mostly in generated icon art:

- repeated-island designs still have too many jumps/trims, especially `flower_daisy_simple`, but current travel attribution points first to outline/accent ordering and blocked connector treatment rather than petal fill ordering
